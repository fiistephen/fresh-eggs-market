      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Field label="Bank account">
            <select value={form.bankAccountId} onChange={(e) => setForm((c) => ({ ...c, bankAccountId: e.target.value }))} className="w-full rounded-md border border-surface-200 px-3 py-2 text-body outline-none focus:ring-2 focus:ring-brand-500">
              {accounts.map((a) => <option key={a.id} value={a.id}>{displayAccountName(a)}</option>)}
            </select>
          </Field>
          <Field label="Filename">
            <input type="text" value={form.filename} onChange={(e) => setForm((c) => ({ ...c, filename: e.target.value }))} className="w-full rounded-md border border-surface-200 px-3 py-2 text-body outline-none focus:ring-2 focus:ring-brand-500" />
          </Field>
        </div>

        <Field label="CSV file">
          <input type="file" accept=".csv,text/csv" onChange={handleFileChange} className="block w-full text-body text-surface-600" />
          {readingFile && <p className="mt-2 text-caption text-surface-500">Reading file…</p>}
        </Field>

        <Field label="Or paste CSV text">
          <textarea value={form.csvText} onChange={(e) => setForm((c) => ({ ...c, csvText: e.target.value }))} rows={12} className="w-full rounded-md border border-surface-200 px-3 py-2 font-mono text-caption outline-none focus:ring-2 focus:ring-brand-500" placeholder="Paste raw Providus statement CSV here" />
        </Field>
