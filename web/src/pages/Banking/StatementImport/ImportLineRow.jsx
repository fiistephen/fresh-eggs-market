...
export default function ImportLineRow({ line, categoryMap, selected, onToggleSelected, onSaved }) {
  ...
  return (
    <tr className="border-b border-gray-50 align-top">
      ...
      <td className="px-4 py-3">
        <select value={draft.selectedCategory} onChange={(e) => setDraft((c) => ({ ...c, selectedCategory: e.target.value }))} disabled={isPosted} className="w-full rounded-lg border border-gray-300 px-2 py-2 text-sm outline-none focus:ring-2 focus:ring-brand-500 disabled:bg-gray-100">
          <option value="">Choose category</option>
          {categories.map((cat) => <option key={cat} value={cat}>{categoryLabel(cat, categoryMap)}</option>)}
        </select>
      </td>
      <td className="px-4 py-3">
        <select value={draft.reviewStatus} onChange={(e) => setDraft((c) => ({ ...c, reviewStatus: e.target.value }))} disabled={isPosted} className="w-full rounded-lg border border-gray-300 px-2 py-2 text-sm outline-none focus:ring-2 focus:ring-brand-500 disabled:bg-gray-100">
      ...
      <td className="px-4 py-3">
        <textarea value={draft.notes} onChange={(e) => setDraft((c) => ({ ...c, notes: e.target.value }))} rows={2} disabled={isPosted} placeholder="Bank name – Depositor name" className="w-full rounded-lg border border-gray-300 px-2 py-2 text-sm outline-none focus:ring-2 focus:ring-brand-500 disabled:bg-gray-100" />
      </td>
      <td className="px-4 py-3">
        {isPosted ? (
          <span className="text-xs font-medium text-green-700">Posted</span>
        ) : (
          <button onClick={saveLine} disabled={saving} className="rounded-lg border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-60">
            {saving ? '…' : 'Save'}
          </button>
        )}
      </td>
