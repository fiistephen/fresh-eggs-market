...
export default function QueueList({ loading, queue, openBatches, policy, onRefresh }) {
  ...
  if (!queue || queue.length === 0) {
    return <EmptyPanel title="No customer deposit money waiting" body="Any customer deposit saved in Banking will show here until the customer and booking allocations are sorted out." />;
  }
  ...
  return (
    <div className="space-y-4">
      <SummaryBanner
        tone="warning"
        title={`${queue.length} customer deposit transaction${queue.length === 1 ? '' : 's'} waiting for action`}
        body="Link the customer, split the money across one or more open batches, and turn the deposit into real bookings."
      />
        ...
        {queue.map((transaction) => (
          <div key={transaction.id} className="rounded-xl border border-surface-200 bg-surface-0 p-5">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <StatusChip
                    label={transaction.category === 'CUSTOMER_DEPOSIT' ? 'Customer deposit' : transaction.category === 'UNALLOCATED_INCOME' ? 'Unallocated income' : 'Legacy customer booking'}
                    tone={transaction.category === 'CUSTOMER_DEPOSIT' ? 'info' : 'default'}
                  />
                </div>
            ...
            <div className="mt-4 flex justify-end">
              ...
              >
                {transaction.customer ? 'Create or match bookings' : 'Link customer and create bookings'}
              </button>
            </div>
          </div>
        ))}
